...
|:---                 |:---
| OS                  | Linux* Ubuntu* 20.04 <br> CentOS* 8 <br> Fedora* 30 <br> SLES 15
...
> we currently focus on displaying how the CLI interface can be used
...
There are 4 files in the solution.

| File          | Description
|:---           |:---
| jacobi.cpp    | Contains the `main` function: <br> * initialize the problem <br> * setup SYCL boiler plate <br> call the `iterate` function <br> validate the result
| bugged.cpp    | Contains bugged versions of `iterate`, `compute_x_k1`, and `prepare_for_next_iteration` functions. <br> This file is included only into `jacoby-bugged` program.
| fixed.cpp     | Contains fixed versions of `iterate`, `compute_x_k1`, and `prepare_for_next_iteration` functions. <br> This file is included only into `jacoby-fixed` program.
| selector.hpp  | Contains helper functions to select the device based on the user-provided arguments.
...
All intended bugs are located in the `bugged.cpp`.
...
## Build the `jacoby-bugged` and `jacoby-fixed` programs
...
3.  Start a debugging session, replacing `<device>` as noted above:
...
For instructions about starting and using the debugger on Linux* OS Host, please see
...
The below instructions provide step by step instructions for locating and resolving 
the three bugs in the `jacobi' sample, as well as basic usage of the debbuger. 
...
The above sets actions for breakpoint 1. At each hit of the breakpoint, the variable 
`var1` will be printed and then the debugger will continue until the next breakpoint hit. 
...
>>>>>>> 76300487... Tools/ApplicationDebugger: update jacobi sample.
...
You can define a convenience variable and use it for computations from the debugger.
...
a common strategy.  Although this sample uses arguments to specify the device, 
you can SYCL device filters on your own applications, for example: 

```
$ SYCL_DEVICE_FILTER=level_zero:gpu ./myApplication`. 
```
